...
func MIPS_mnist_model_load(model *mnist_model) error {
	...
	// Read Fc2 layer 2
	{
		...
		model.hparams.n_hidden2 = ne_weight[1]
		//model.hparams.n_classes = ne_weight[1]

		if READ_FROM_BIDENDIAN {
			fc2_weight_data_size := int(model.hparams.n_hidden * model.hparams.n_hidden2)
			fc2_weight_data := common.DecodeFloat32List(model_bytes[index : index+4*fc2_weight_data_size])
			index += 4 * fc2_weight_data_size
			model.fc2_weight = ml.NewTensor2DWithData(nil, ml.TYPE_F32, uint32(model.hparams.n_hidden), uint32(model.hparams.n_hidden2), fc2_weight_data)
		} else {
			model.fc2_weight = ml.NewTensor2D(nil, ml.TYPE_F32, uint32(model.hparams.n_hidden), uint32(model.hparams.n_hidden2))
		...
		if READ_FROM_BIDENDIAN {
			fc2_bias_data_size := int(model.hparams.n_hidden2)
			fc2_bias_data := common.DecodeFloat32List(model_bytes[index : index+4*fc2_bias_data_size])
			index += 4 * fc2_bias_data_size
			model.fc2_bias = ml.NewTensor1DWithData(nil, ml.TYPE_F32, uint32(model.hparams.n_hidden2), fc2_bias_data)
		} else {
			model.fc2_bias = ml.NewTensor1D(nil, ml.TYPE_F32, uint32(model.hparams.n_hidden2))
	...
	// Read Fc3 layer 3
	{
		...
		if READ_FROM_BIDENDIAN {
			fc3_weight_data_size := int(model.hparams.n_hidden2 * model.hparams.n_classes)
			fc3_weight_data := common.DecodeFloat32List(model_bytes[index : index+4*fc3_weight_data_size])
			index += 4 * fc3_weight_data_size
			model.fc3_weight = ml.NewTensor2DWithData(nil, ml.TYPE_F32, uint32(model.hparams.n_hidden2), uint32(model.hparams.n_classes), fc3_weight_data)
		} else {
			model.fc3_weight = ml.NewTensor2D(nil, ml.TYPE_F32, uint32(model.hparams.n_hidden2), uint32(model.hparams.n_classes))
			for i := 0; i < len(model.fc3_weight.Data); i++ {
				model.fc3_weight.Data[i] = common.ReadFP32FromBytes(model_bytes, &index, READ_FROM_BIDENDIAN)
			}
		}
		...
		if READ_FROM_BIDENDIAN {
			fc3_bias_data_size := int(model.hparams.n_classes)
			fc3_bias_data := common.DecodeFloat32List(model_bytes[index : index+4*fc3_bias_data_size])
			index += 4 * fc3_bias_data_size
			model.fc3_bias = ml.NewTensor1DWithData(nil, ml.TYPE_F32, uint32(model.hparams.n_classes), fc3_bias_data)
		} else {
			model.fc3_bias = ml.NewTensor1D(nil, ml.TYPE_F32, uint32(model.hparams.n_classes))
			for i := 0; i < len(model.fc3_bias.Data); i++ {
				model.fc3_bias.Data[i] = common.ReadFP32FromBytes(model_bytes, &index, READ_FROM_BIDENDIAN)
			}
		}

		ml.PrintTensor(model.fc3_bias, "model.fc3_bias")
	}
...
func MIPS_InputProcess() []float32 {
	...
	for row := 0; row < 14; row++ {
		for col := 0; col < 14; col++ {
			digits[row*14+col] = float32(buf[row*14+col])
			if buf[row*14+col] > 230 {
				c += "*"
			} else {
				c += "_"
			}
...
func MIPS_mnist_eval(model *mnist_model, digit []float32) int {
	...
	fc3 := ml.Add(ctx0, ml.MulMat(ctx0, model.fc3_weight, ml.Relu(ctx0, fc2)), model.fc3_bias)

	// softmax
	final := ml.SoftMax(ctx0, fc3)
